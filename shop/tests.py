...
class ShopAPITestCase(APITestCase):
    ...
    def test_get_cart_with_total_price(self):
        """
        Проверка: получение корзины (GET /cart/) и корректный подсчет
        общей стоимости (total_cart_price).
        """
        self.client.force_authenticate(user=self.user)
        ...
        self.assertIn("total_cart_price", response.data)
        self.assertIn("items", response.data)

        expected_total = Decimal("160000.00")
        actual_total = Decimal(str(response.data["total_cart_price"]))

        self.assertEqual(actual_total, expected_total)

        self.assertEqual(len(response.data["items"]), 1)
        self.assertEqual(
            response.data["items"][0]["product"]["name"], "Смартфон Galaxy S23"
        )
